...
'''
Core utility functions used in ibus-typing-booster.
...
UNICODE_DATA_EXTRA_NAMES = {
    '''
    A few names for Unicode characters which don’t have
    any official names (That includes most control characters).

    EmojiMatcher loads a more complete list of names, but
    EmojiMatcher might not be loaded in Typing Booster if
    emoji predictions are off and loading it only for these
    names seems overkill.
    '''
    '\u0000': 'NULL',
    '\u0001': 'START OF HEADING',
    '\u0002': 'START OF TEXT',
...
def unicode_name(character: str) -> str:
    '''
    Get a name for an Unicode character

    :param character: The character to get the name for.
    :return:          The name from unicodedata.name(character)
                      or a fallback.
    ...
    if not character or len(character) > 1:
        return ''
    try:
        result = unicodedata.name(character)
    except ValueError:
        result = UNICODE_DATA_EXTRA_NAMES.get(character, '')
    return result
...
COMPOSITION_EXCLUSIONS = {
    ...
    # Do not do this for Hebrew,
    # See: https://github.com/mike-fabian/ibus-typing-booster/issues/716
    # and https://corp.unicode.org/pipermail/unicode/2024-October/011095.html
